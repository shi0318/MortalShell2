---
title: "Gloom in Mortal Shell II — Trailer Clues & Open Questions"
heading: "The Gloom mechanic in Mortal Shell II"
description: "What Mortal Shell II trailers show about Gloom so far, what remains unconfirmed, and how it may relate to systems from the first game."
category: mechanics
keyword: "mortal shell 2 gloom"
cover: /images/mortalshell2-gloom-spider-creature.webp
status: trailer
preRelease: true
order: 20
updatedAt: "2026-07-23"
sources:
  - status: trailer
    note: "Gloom effect featured in the Official Reveal Trailer marketing."
    lastChecked: "2026-07-23"
  - status: prequel
    note: "Comparison drawn to the Harden and resolve systems of Mortal Shell (2020). Marked as inference, not confirmation."
    lastChecked: "2026-07-23"
---

## What is Gloom?

**Gloom** is a mechanic featured prominently in Mortal Shell II's reveal marketing. As of 2026-07-23, its exact rules have **not** been detailed by the developer. What follows separates what the trailer shows (👁️) from what we are inferring based on the first game (📖).

This page remains a historical trailer reference. Retail Gloom details belong in a separate current-build record rather than being inferred from the reveal footage.

## What the trailer shows

![Gloom-wreathed spider-like creature in a foggy Mortal Shell II swamp](/images/mortalshell2-gloom-spider-creature.webp)

👁️ **Seen in the trailer:**

- A visible "Gloom" effect wreathing certain enemies and areas
- The protagonist appearing to interact with or push back the Gloom during combat

The trailer does not explain the resource cost, activation, or benefit of the mechanic.

## Prequel context (inference, not confirmation)

📖 **Based on Mortal Shell (2020):**

The first game built its identity on two systems that Gloom may extend or replace:

- **Harden** — turning to stone to negate an incoming hit
- **Resolve** — a resource spent on weapon abilities and parries

It is reasonable to expect Gloom to interact with one of these, but we are explicitly **not** claiming that. This remains prequel-based inference, not retail data.

## Retail details still needed for a complete Gloom guide

- The precise activation and resource cost of Gloom
- Whether it is offensive, defensive, or both
- Which enemies and areas it applies to
- Any build or Shell synergies

Check the confirmation badge at the top of this page: this article is Trailer evidence, not a retail mechanics table.


## Related official combat note

The official Mortal Shell II site states combat is **unrestricted by stamina**, with posture-breaking tactics and **sidearms**. That is a full-game framing note, not yet a complete Gloom economy document. Until the live client confirms currency sinks and drop rules, treat Gloom upgrade details as provisional.
